...
def fetch_series_markets(client: KalshiClient, series_ticker: str) -> list:
    ...
    try:
        ...
        for market_data in data.get("markets", []):
            try:
                markets.append(
                    KalshiMarket.from_api_response(market_data, client.price_source)
                )
            except Exception:
                pass
        return markets
...
def analyze_market(
    market, weather_client: WeatherClient, probability_model: WeatherProbabilityModel
):
    ...
    # Liquidity warning
    liq_status = (
        "✓ Active" if getattr(market, "has_liquidity", True) else "⚠️  LOW/STALE"
    )
    ...
    print(f"Question:    {market.question}")
    print(
        f"Link:        https://kalshi.com/markets/{market.market_id.split('-')[0].lower()}"
    )
    print("-" * 70)
    ...
    print("-" * 70)
    print(
        f"Action:      {action_color}{action}{reset}"
        if abs(result.edge) >= 0.05
        else f"Action:      HOLD (edge < 5%)"
    )
...
def main():
    ...
    print(
        ...
        + (f" / {market_ticker}" if market_ticker else " (all markets)")
    )
    ...
    if market_ticker and "-" in market_ticker and len(market_ticker.split("-")) >= 2:
        ...
        markets = [
            m for m in markets if m.market_id.upper().startswith(market_ticker.upper())
        ]
        if not markets:
    ...
    if len(results) > 1:
        ...
        print("=" * 70)
        print(
            f"{'Market':<30} | {'Mkt':>5} | {'Fair':>5} | {'Edge':>7} | {'Action':<8}"
        )
        print("-" * 70)
        ...
        for r in results:
            ...
            print(
                f"{market_id:<30} | {r.market_prob:>4.0%} | {r.fair_prob:>4.0%} | {r.edge:>+6.1%} | {action:<8}"
            )
