...
class WeatherProbabilityModel:
    ...
    REPORTING_NOISE = {
        "San Francisco": 2.5, "Los Angeles": 2.0, "Miami": 2.0,
        "Boston": 1.8, "Seattle": 1.8, "New Orleans": 1.8,
        "New York": 1.5, "Philadelphia": 1.5, "Atlanta": 1.5,
        "Chicago": 1.5, "Minneapolis": 1.5, "Houston": 1.5,
        "Dallas": 1.5, "Denver": 1.5, "Phoenix": 1.2, "Las Vegas": 1.2
    }
    DEFAULT_NOISE = 1.5
    ...
    def evaluate_market(self, market: KalshiMarket) -> EdgeResult | None:
        ...
        if len(ticker_parts) >= 2:
            try:
                weather_date = datetime.strptime(ticker_parts[1], "%y%b%d").replace(tzinfo=timezone.utc)
            except:
        ...
        if weather.current_temp_f is not None:
            ...
            if "high" in params["threshold_type"]:
                if params.get("is_bucket"):
                    if current >= params["upper_bound"]: fair_prob = 0.0
                    elif current >= params["lower_bound"]: fair_prob = max(fair_prob, 0.7)
                else:
                    thresh = params["threshold_temp"]
                    if "above" in params["threshold_type"] and current > thresh: fair_prob = 1.0
    ...
    def _calculate_bucket_probability(self, weather, params):
        mu = weather.high_temp_f if "high" in params["threshold_type"] else weather.low_temp_f
        std = self._get_effective_std(weather.high_temp_std, weather.location)
        k = int(params["lower_bound"])
        return self._normal_cdf((k + 0.5 - mu)/std) - self._normal_cdf((k - 0.5 - mu)/std)
